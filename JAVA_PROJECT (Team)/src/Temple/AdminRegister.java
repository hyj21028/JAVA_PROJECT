...
public class AdminRegister extends JFrame {
	...
	public AdminRegister(LoginFrame loginFrame) {
		...
		lanameWarn.setBounds(328, 299, 178, 15);
		...
		laposWarn.setBounds(328, 243, 178, 15);
		...
		BtnSave.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cmd.trim().equals("Save")) {
					...
					JOptionPane.showMessageDialog(null, "가입되지 않았습니다.");
				}
				// dispose();
		...
		BtnIdCheck.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cmd.trim().equals("Check")) {
					...
					DBconn dBconn = DBconn.getInstance();
					int n = dBconn.confirm1(adminId);

					if (n == 1) {
						init();
						JOptionPane.showMessageDialog(null, "아이디가 일치합니다.");
					} else {
						JOptionPane.showMessageDialog(null, "일치하지않습니다..");
